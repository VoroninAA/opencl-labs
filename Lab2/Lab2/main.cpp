...
void saxpy_gpu(size_t n, float a, float* x, int incx, float* y, int incy) {
    ...
    cl_mem ys_buff = clCreateBuffer(context, CL_MEM_READ_ONLY, sizeof(float) * n * incy, nullptr, nullptr);
    CHK(ys_buff);
...
void daxpy_gpu(size_t n, double a, double* x, int incx, double* y, int incy) {
    ...
    cl_mem xs_buff = clCreateBuffer(context, CL_MEM_WRITE_ONLY, sizeof(double) * n * incx, nullptr, nullptr);
    CHK(xs_buff);

    cl_mem ys_buff = clCreateBuffer(context, CL_MEM_READ_ONLY, sizeof(double) * n * incy, nullptr, nullptr);
    CHK(ys_buff);
