...
void matrix_multiply_gpu_buffers(const Matrix& a, const Matrix& b, Matrix& res, const char* program_name) {
    ...
    cl_mem a_buff = clCreateBuffer(context, CL_MEM_READ_ONLY, sizeof(int) * a.width * a.height, nullptr, nullptr);
    CHK(a_buff);

    cl_mem b_buff = clCreateBuffer(context, CL_MEM_READ_ONLY, sizeof(int) * b.width * b.height, nullptr, nullptr);
    CHK(b_buff);

    cl_mem res_buff = clCreateBuffer(context, CL_MEM_WRITE_ONLY, sizeof(int) * res.width * res.height, nullptr, nullptr);
    CHK(res_buff);
...
void matrix_multiply_gpu_images(const Matrix& a, const Matrix& b, Matrix& res, const char* program_name) {
    ...
    cl_mem a_buff = clCreateImage2D(context, CL_MEM_READ_ONLY, &form, a.width, a.height, 0, nullptr, nullptr);
    CHK(a_buff);

    cl_mem b_buff = clCreateImage2D(context, CL_MEM_READ_ONLY, &form, b.width, b.height, 0, nullptr, nullptr);
    CHK(b_buff);

    cl_mem res_buff = clCreateImage2D(context, CL_MEM_WRITE_ONLY, &form, res.width, res.height, 0, nullptr, nullptr);
    CHK(res_buff);
...
void matrix_test() {
    ...
    // constexpr int n = 400, m = 320, l = 400;
